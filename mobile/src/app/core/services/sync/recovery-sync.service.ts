...
import { DefaultDailyStakeRequest, SpecialDailyStakeRequest } from '../../../models/sync.model';
...
export class RecoverySyncService extends BaseSyncService<Recovery, RecoveryRepository> {
    ...
    private async syncSpecialDailyStakes(recoveries: Recovery[]): Promise<void> {
        ...
        try {
            const response = await firstValueFrom(
                this.http.post<ApiResponse<string[]>>(`${this.baseUrl}/api/v1/credits/special-daily-stake`, syncRequest, { headers })
            );
