#!/usr/bin/env bash
# Installe le plugin natif AppUpdate (APK install + SHA-256) dans le projet Android Capacitor.
set -euo pipefail

ANDROID_DIR="${1:?android directory required}"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
CONFIG_DIR="$SCRIPT_DIR/../workflows/android-config-prod"

JAVA_PKG_DIR="$ANDROID_DIR/app/src/main/java/com/optimize/elykia"
