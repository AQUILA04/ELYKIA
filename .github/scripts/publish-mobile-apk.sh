...
if [[ -n "${MINIO_MOBILE_RELEASES_BUCKET:-}" ]]; then
  MINIO_BUCKET="$MINIO_MOBILE_RELEASES_BUCKET"
elif [[ "$TARGET" == "test" ]]; then
  MINIO_BUCKET="elykia-mobile-releases-test"
else
  MINIO_BUCKET="elykia-mobile-releases"
fi
...
ensure_bucket() {
  ...
  echo "Create it in the MinIO/Contabo console (expected: elykia-mobile-releases[-test])." >&2
  return 1
}
ensure_bucket "elykia-release/${MINIO_BUCKET}"
