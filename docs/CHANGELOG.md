...
- **GitHub Actions Node 24** : `actions/upload-artifact@v4` → `@v6` dans `ci.yml` (JaCoCo) ; `actions/checkout@v4` → `@v6` dans `deploy-landing-page.yml` et les workflows Android debug mobile — fin de l’annotation de dépréciation Node 20 sur le runtime des actions.
...
- Activité récente : logs retour de stock (et tontine) enrichis avec le commercial, le montant total et le nombre d’articles ; logs versement avec le commercial concerné.
...
- **User Guide (Frontend Web - Stocks, Réceptions & Inventaires)** : Refonte complète de `user-guide/docs/manager/stock_sales.md`, `user-guide/docs/commercial/stock.md` et des guides Magasinier (`storekeeper/inventory.md`, `storekeeper/articles.md`, `storekeeper/stock_commercial.md`, `storekeeper/stock_tontine.md`) :
  1. *Architecture à 3 niveaux de stock* : Stock Central Magasin (dépôt), Stock Commercial (agents terrain) et Stock Tontine étanche (réservé aux livraisons de fin d'année des cotisants).
  2. *Entrées fournisseurs & Historique des réceptions (`/stock/receptions`)* : Saisie d'entrée, référence unique, statut initial `PENDING` sans impact stock, contrôle et validation obligatoire par le gestionnaire (`VALIDATED` augmentant le stock magasin), actions `Refuser`, `Abandonner` ou `Annuler`.
  3. *Demandes de sortie de stock commercial (`/stock/request`)* : Cycle complet en 3 temps (`CREATED` création commercial/gestionnaire $\rightarrow$ `VALIDATED` validation par gestionnaire $\rightarrow$ `DELIVERED` remise physique par magasinier avec débit magasin et crédit stock commercial), exports PDF unitaires et groupés par sélection.
  4. *Tableau de bord « Mon Stock » mensuel (`/stock/my-stock`)* : Accordéon par mois civil et commercial, 6 KPIs financiers de contrôle (Valeur stock restant, Valeur stock vendu avec carte cliquable ouvrant le détail des ventes liées, Valeur totale due, Montant recouvré, Reste à recouvrer, Taux de recouvrement %), table détaillée des mouvements par article (Pris, Vendu, Retourné, Restant, Valorisation PMP), et bouton de retour sur stock antérieur (`/stock/return/historique`).
  5. *Retours de marchandise (`/stock/return`)* : Déclaration avec motif (`PENDING`), contrôle physique et clic `Réceptionner` par le magasinier (`RECEIVED` réapprovisionnant le magasin central et déchargeant le commercial) ou refus.
  6. *Inventaires physiques & Réconciliation (`/inventory/list`)* : 5 indicateurs gestionnaire de valorisation (Achat FIFO, Vente crédit, Marge crédit, Vente comptant, Marge comptant), session active `IN_PROGRESS`, feuille de comptage en aveugle PDF, saisie des quantités réelles, calcul des écarts (surplus/manquants), motifs et clôture définitive réalignant le stock théorique.
  7. *Rôle Magasinier* : Réceptions, préparation physique des commandes, contrôle contradictoire au comptoir, alertes stock (Point de commande et Seuil d'alerte).
- **User Guide (Frontend Web - Ventes, Crédits, Recouvrements & Commandes)** : Refonte exhaustive de la documentation commerciale et du suivi de crédit dans `user-guide/docs/commercial/sales_orders.md` :
  1. *Liste des ventes & KPIs (`/credit/list`)* : Bandeau financier (`ROLE_KPI_FINANCIER_VENTE` : Total ventes, Montant total, Montant encaissé, Solde restant, Taux de recouvrement), filtres temporels rapides (Aujourd'hui, Cette semaine, Ce mois, Personnalisée), recherche avancée multi-critères, badges de criticité d'échéance colorés (vert > 5j, orange 1-5j, rouge 0j/dépassé).
  2. *Cycle de vie des ventes* : Statuts et actions associées (`CREATED` avec Valider/Modifier/Supprimer, `VALIDATED` avec Démarrer remise matériel et sortie de stock, `INPROGRESS` avec Encaisser/Modifier la mise, `SETTLED` soldé).
  3. *Opérations en lot & Fusion* : Réaffectation commerciale groupée (`ROLE_ASSIGN_CLIENT_COLLECTOR`) et modal de fusion de crédits (consolidation de 2 à 10 crédits d'un même commercial en un crédit unique).
  4. *Nouvelle vente (`/credit/add`)* : Dualité Vente au Comptant (`CASH` avec prix comptant, lazy-loading articles, sans commercial, impression ticket) vs Vente à Crédit (`CREDIT` avec sélection commercial verrouillée pour promoteur, clients filtrés, stock commercial disponible, prix crédit, calcul d'échéancier, avance, et finalité Personnel/Professionnel selon badge B et feature flag).
  5. *Impression reçu thermique* : Modal d'impression automatique post-vente 80mm récapitulant les coordonnées, articles, acompte, solde restant et mise journalière.
  6. *Fiche 360° Détail du Crédit (`/credit/details/:id`)* : Progression visuelle du remboursement (%, payé vs total, restant dû, jours restants), 4 KPIs financiers, fiche client avec solde reliquat mis en avant (vert si > 0), bloc commercial avec bouton de réassignation individuelle et historique des transferts, lien direct vers le stock mensuel source, codes de consentement numérique, contrôle terrain contradictoire (système vs carnet, écart, statut Conforme/Disparité), tableau desktop et cartes mobile des articles, bandeau d'évaluation de retard avec 4 compteurs (écoulés, payés, retard, restants), timeline des recouvrements avec annulation autorisée (`ROLE_CANCEL_RECOVERY`), et historique des changements de mise journalière.
  7. *Retards & Échéances (`/credit/late`, `/credit/echeance`)* : KPIs de retard (`ROLE_KPI_FINANCIER_RETARD`), clôture exceptionnelle de crédit compromis, contrôle terrain direct, export PDF de tournée, et tableau des échéances du jour et de la semaine (`ROLE_KPI_FINANCIER_ECHEANCE`).
  8. *Journal des recouvrements (`/credit/recouvrements`)* : Suivi chronologique des encaissements, KPIs de collecte, filtrage commercial et annulation d'encaissement avec recalcul automatique d'intégrité comptable.
  9. *Rapport de transfert des ventes (`/credit/transferts-commerciaux`)* : Matrice des passations entre commerciaux (`ROLE_KPI_FINANCIER_TRANSFERT_VENTE`), agrégats financiers par paire d'agents (`oldCollector → newCollector`), et règle stricte de déduplication sur la dernière passation.
  10. *Rattrapages de ventes (`/stock/credit/rattrapage`)* : Procédure exceptionnelle (`ROLE_RATTRAPAGE`) injectant des ventes passées adossées à un stock mensuel résiduel d'un agent sans déduire du stock physique magasin actuel.
  11. *Articles vendus (`/credit/articles-vendus`)* : Tableau de bord consolidé des volumes et montants vendus groupés par article et par commercial.
  12. *Pipeline des Commandes (`/orders`)* : Tableau de bord avec onglets par statut (`PENDING`, `ACCEPTED`, `DENIED`, `CANCEL`, `SOLD`, `ALL`), 6 KPIs de pipeline (en attente, valeur potentielle, taux d'acceptation, pipeline accepté, marge potentielle, croissance), formulaire de commande (`/orders/create`), détail (`/orders/details/:id`), approbation, et conversion directe en vente effective via l'action « Vendre ».
- **User Guide (Frontend Web - Tontines Web)** : Refonte complète et enrichissement de la documentation du module Tontine dans `user-guide/docs/commercial/tontine.md` :
  1. *Cycle annuel et sessions (`/tontine`)* : Cycle de 10 mois calendaires (février à novembre), session Active (`ACTIVE` avec pleins droits d'écriture) vs sessions Historiques (`CLOSED`/`ENDED` en lecture seule sécurisée avec alerte visuelle), et outil de comparaison pluriannuelle de sessions (`/tontine/compare`).
  2. *Bandeau KPIs Financiers Tontine (`ROLE_KPI_FINANCIER_TONTINE`)* : Membres actifs, total collecté, revenu total part société, livraisons en attente & livrées, contribution moyenne, et collectes perçues à la livraison.
  3. *Enrôlement & Vérification des carnets* : Inscription unitaire (`AddMemberModalComponent`) et en lot (`AddMultipleMembersModalComponent`), vérification groupée des carnets physiques avec horodatage et auditeur (`ROLE_TONTINE_CARNET_VERIFY`), et exports PDF ciblés par commercial et par statut de vérification de carnet.
  4. *Fiche Membre 360° (`/tontine/member/:id`)* : Vue intégrale, 4 KPIs financiers (total contribué, solde disponible net, part société payé vs dû théorique calculé, collectes livraison), répartition des cotisations par commercial avec mise en avant de l'agent actuel, contrôle terrain contradictoire (système vs carnet, écart, statut Conforme/Disparité et décomposition mensuelle), grille visuelle de progression des 10 mois (icônes validées pour 31 jours cotisés, jauge mois courant), synthèse mensuelle avec pastilles de jours acquis, historique des changements de mise journalière, codes de consentement, et téléchargement d'attestation PDF.
  5. *Cotisations & Droit à l'erreur* : Enregistrement de collecte du jour vs collecte de rattrapage sur date passée (`RecordCatchupCollectionModalComponent`) avec mise applicable et prévisualisation dynamique, et annulation de collecte avec recalcul immédiat d'intégrité financière (`ROLE_CANCEL_TONTINE_COLLECTION`).
  6. *Livraisons d'articles de fin d'année (`/tontine/livraisons`)* : Pipeline complet (`SESSION_INPROGRESS` $\rightarrow$ `PENDING` préparation du panier plafonné au solde disponible $\rightarrow$ `VALIDATED` approbation administrative $\rightarrow$ `DELIVERED` remise physique avec déstockage du Stock Tontine), gestion du solde non utilisé résiduel, et tableau de bord avec KPIs de livraison.
  7. *Journal des collectes (`/tontine/collectes`)* : Suivi exhaustif des encaissements par période et commercial.
  8. *Archives & Réinitialisation de campagne (`/tontine/reset-collectes`)* : Arborescence hiérarchique Année $\rightarrow$ Run $\rightarrow$ Fichiers PDF par commercial et quartier, action d'archivage simple, et action de réinitialisation annuelle de fin de campagne remettant à zéro les contributions de la session tout en préservant le fichier des membres.
- **User Guide (Frontend Web - Paiements Clients, Mobile Money & Notifications)** : Documentation opérationnelle et fonctionnelle complète dans `user-guide/docs/manager/operations.md`, `user-guide/docs/manager/reporting_config.md`, `user-guide/docs/commercial/sales_orders.md` et `user-guide/docs/commercial/tontine.md` :
  1. *Paiements clients en ligne (`/customer-payments`)* : Arbitrage des déclarations de paiement Mobile Money initiées par les clients depuis l'Espace Client ELYKIA, barre d'état et d'actualisation en temps réel, compteur dynamique des déclarations en attente, et distinction fonctionnelle des portefeuilles (gestionnaire = vue globale agence, commercial = clients assignés).
  2. *Onglet Recouvrement crédit* : Tableau des déclarations (client, commercial gestionnaire, lien cliquable vers le détail du crédit `#ID`, numéro d'échéance, montant en Francs CFA, numéro de téléphone, référence transaction opérateur), bouton de validation immédiate imputant le crédit et incrémentant la caisse/recouvrement du jour, et bouton de rejet avec boîte de dialogue de confirmation.
  3. *Onglet Cotisations tontine* : Tableau des cotisations d'épargne déclarées (client, commercial tontine, numéro de membre tontine `#ID`, montant, téléphone, référence), bouton de validation créditant la cagnotte active avec calcul automatique de la part société selon barème, et bouton de rejet.
  4. *Centre de Notifications (`/notifications`)* : Cloche d'en-tête avec pastille dynamique de non-lus (`unreadCount`), menu contextuel déroulant groupé par date, lien direct marquant l'alerte lue et ouvrant l'opération cible avec mise en surbrillance de ligne (`highlightId`), page centrale avec actions « Tout lire » et « Actualiser », et typologie des alertes (Paiement crédit, Cotisation tontine, Commande client, Rattrapage).
  5. *Configuration Mobile Money (`/configuration/mobile-money`)* : Paramétrage des numéros de transfert d'argent Mixx by YAS et Moov Money attribués à chaque commercial de l'agence, affichage des numéros globaux entreprise par défaut, KPIs de synthèse (commerciaux configurables et configurations spécifiques actives), tableau des commerciaux, colonne dynamique d'effectivité (application prioritaire du numéro spécifique ou repli automatique sur le numéro global de la société en cas de champ vide), et bouton d'enregistrement par ligne.
- **User Guide (Frontend Web - Finances, Dépenses & Remises)** : Refonte intégrale et enrichissement de la documentation financière et de trésorerie dans `user-guide/docs/manager/finance.md` :
  1. *Tableau de bord des dépenses (`/expense/dashboard`)* : Horloge en temps réel, horodatage d'actualisation, KPIs comparatifs plurimensuels des décaissements, raccourcis vers la création et les types de dépense, et tableau paginé des 10 dernières dépenses avec modal de consultation détaillée.
  2. *Registre des dépenses (`/expense/list`)* : Filtres combinables par mois civil et type de charge, colonnes date, montant en Francs CFA, description et référence de justificatif.
  3. *Verrou d'intégrité Comptabilisée* : Explicitation du badge vert « Comptabilisée » désactivant formellement l'édition et la suppression des dépenses déjà intégrées dans une remise de trésorerie validée.
  4. *Saisie d'une dépense (`/expense/add`)* : Formulaire avec type obligatoire, montant positif, date, référence de pièce et description justificative.
  5. *Remise au gestionnaire (`Rapport Journalier > Remise`)* : Cycle complet de versement de caisse entre secrétaire et gestionnaire d'agence sur une plage de dates personnalisée (*Du / Au*).
  6. *6 Indicateurs financiers de remise* : Reste à remettre brut (avec historique *Déjà remis*), ventilation crédit, cotisations tontine, dépôts de nouveaux comptes, cumul des dépenses déduites, et Montant net à remettre en espèces avec alerte bloquante rouge en cas de solde négatif.
  7. *Sélection et arbitrage des dépenses* : Déduction sélective des dépenses par case à cocher, exclusion stricte des dépenses d'approvisionnement, possibilité pour le gestionnaire de retirer une dépense contestée avant réception.
  8. *Cycle d'approbation et accusé de réception* : Statut initial `PENDING` lors de la soumission secrétaire, vérification contradictoire des espèces et pièces au bureau, et bouton « Accuser réception » (ou « Initier la réception » directe par le gestionnaire) figeant irrévocablement la remise (`SETTLED`) et passant les dépenses associées au statut comptabilisé.
  9. *Historique des remises* : Journal chronologique avec accordéon déroulant dévoilant l'ensemble des versements unitaires inclus.
- **User Guide (Frontend Web - Rapports d'activité, ELYKIA IA & Paramètres)** : Refonte complète et enrichissement de la documentation dans `user-guide/docs/manager/reporting_config.md` :
  1. *Rapport Journalier d'activité (`/report/daily`)* : 5 onglets opérationnels unifiés sous les filtres temporels (Aujourd'hui, Cette semaine, Ce mois, Personnalisé Du/Au) et filtre commercial (portefeuille personnel pour le commercial, vision globale d'agence ou ciblée pour le gestionnaire), et bascule d'affichage des marges commerciales brutes.
  2. *Bilan Annuel Commercial* : Bilans annuels crédit (portefeuille confié, stock d'ouverture au 01/01, ventes cumulées, passations reçues/cédées, versements crédit, reste chez le commercial et reste chez le client avec détail des débiteurs et export PDF) et tontine (collectes, versements remis, solde restant).
  3. *Segments Journal, Recouvrement & Versements* : Journal chronologique de toutes les opérations avec export PDF, suivi du recouvrement terrain (taux de recouvrement, montant à remettre par commercial, bordereau PDF) et traçabilité des versements de caisse ventilés par provenance (crédit, tontine, solde nouveaux comptes).
  4. *Rapports Mensuels Consolidés (`/report/monthly-reports`)* : Arborescence hiérarchique d'archives officielles (Année $\rightarrow$ Mois $\rightarrow$ Fichiers PDF), téléchargement 1 clic du Rapport Général de l'agence et des rapports individuels par commercial, et action « Générer mois précédent » à la demande.
  5. *Assistant Décisionnel ELYKIA IA (`/ai-chat`)* : Requêtes en langage naturel sur la base de données opérationnelle (Text-to-SQL sécurisé), volet d'historique des discussions, suggestions de prompts métier par domaine (Ventes, Crédits, Tontines, Stocks, Caisse), réponses tabulaires structurées, bouton d'affichage de la requête d'audit et temps de calcul, et métriques d'usage dans l'onglet Statistiques.
  6. *Référentiels et Paramètres d'agence (`/configuration`)* : Gestion des localités/quartiers (`/locality`), types d'articles (`/article-type`), types de dépense (`/expense/types`), numéros Mobile Money (`/configuration/mobile-money`) et paramètres généraux système (`/parameters` - dont version de calcul de part société tontine V1/V2).
  7. *Profil utilisateur & Sécurité* : Menu profil en barre supérieure (coordonnées, agence, changement de mot de passe) et déconnexion sécurisée obligatoire.
- **Harmonisation fonctionnelle & Suppression des codes techniques** : Élimination intégrale des codes techniques de permission (`ROLE_*`) sur l'ensemble de la documentation utilisateur au profit d'un langage fonctionnel et pédagogique d'auto-formation :
  1. *Habilitations* : Remplacement des vérifications techniques par l'explication naturelle de l'absence d'éléments (« si vous ne voyez pas ce bouton ou cet indicateur, vous ne disposez pas des habilitations nécessaires »).
  2. *Vocabulaire standard* : Adoption exclusive du terme **Commercial** en remplacement des dénominations techniques de code (promoteur, collecteur, collector, promotor).
  3. *Visibilité des données par profil* : Explicitation pédagogique des périmètres d'accès (le commercial ne visualise que ses propres données, tandis que le gestionnaire bénéficie d'une vision globale d'agence avec filtre par agent).
- **Générateur & RAG Index** : Compilation de l'ensemble des guides imprimables (`guide_complet_manager.md`, `guide_complet_commercial.md`, `guide_complet_storekeeper.md`, `guide_complet_recovery_manager.md`) et mise à jour de l'index RAG IA (107 fragments) dans `backend/src/main/resources/ai/user-guide-index.json`.

## Docs & Infra — 2026-09-21

### Added

- **User Guide (Application Mobile - Restructuration & Tontine)** : Réorganisation logique et pédagogique intégrale du guide de l'application mobile terrain dans `user-guide/docs/commercial/mobile_app.md` alignée sur le flux de travail naturel de l'utilisateur :
  1. *Tableau de bord d'accueil* : profil, connectivité En ligne/Hors ligne, période (Jour/Semaine/Mois/Année), 6 KPIs financiers, graphique interactif de tendances, grille des 6 actions rapides (*Nouvelle Distribution*, *Recouvrement*, *Nouveau Client*, *Rapport*, *Tontine*, *Stock*), et barre inférieure permanente d'onglets (Tabs).
  2. *Module Clients* (Mes clientes) : recherche, filtres chips, menu contextuel vers tournée de recouvrement, fiche à 3 onglets, appel direct 1 clic, reliquat, carte Leaflet GPS, formulaire complet avec photos, contrôle 18 ans et capture GPS.
  3. *Module Distribution* (Ventes à crédit) : consultation, modification/suppression locale avec réintégration immédiate du stock, règles financières de 30 jours, plancher 200 FCFA, arrondi 50 FCFA, avance résiduelle, reçu thermique Bluetooth avec QR code.
  4. *Module Recouvrement* : tournée « Clients à recouvrer » par quartier, pastilles de mise, reliquat, double confirmation, code de consentement, reçu thermique Bluetooth, suppression locale.
  5. *Module Tontine (Épargne collective)* : Dashboard avec 4 KPIs et filtre stratégique « À faire » (`todo`) groupé par quartier pour la tournée des cotisations non payées ; formulaire d'adhésion membre avec fréquence, montant, contrôle d'unicité et sélecteur de portée de modification (`updateScope`) ; fiche détaillée membre avec historique chronologique et droit à l'erreur (suppression de cotisation locale) ; saisie de cotisation avec double confirmation et reçu thermique Bluetooth avec cumul à date ; livraison de fin d'année avec déduction de la part société (V1/V2), bouton « Compléter le solde » en cas de surplus panier, double modalité Commande (`ORDER` - PENDING) vs Livraison directe (`DIRECT` - DELIVERED avec sortie de stock), et validation ultérieure « Marquer comme livré ».
  6. *Articles, commandes clients et gestion des stocks* : consultation des articles (double segment *Mon Stock* avec quantités réelles vs *Catalogue* général), commandes clients sur catalogue (réservation sans blocage du stock physique embarqué, création, détail, modification et suppression), et gestion logistique des stocks avec le magasin central (sélecteur de contexte *Standard* / *Tontine*, double flux *Demandes de réapprovisionnement* et *Retours de stock avec motif*, suivi des statuts *En attente/Validé/Livré* et annulation locale d'une opération PENDING).
  7. *Synchronisation hybride et fonctionnement hors-ligne* : architecture locale-d'abord (SQLite, badges Local/Sync, compteur en attente), protocole de sécurité et consentement obligatoire en 3 étapes (mot de passe, code challenge à recopier, engagement de responsabilité) avec vérification préalable de caisse ouverte, écran de synchronisation manuelle multi-onglets (*Clients, Distributions, Recouvrements, Membres, Collectes, Livraisons*) avec sélection globale/unitaire et bouton d'édition du parent pour résoudre les dépendances bloquées, gestionnaire des erreurs de synchronisation avec diagnostic technique JSON (requête, réponse, entité locale), et réglages réseau (synchronisation automatique, désactivation des photos lourdes data, fréquence et filtre d'historique).
  8. *Clôture journalière, rapport d'activité et versement de caisse* : bilan de fin de tournée via le module `rapport-journalier` (6 indicateurs synthétiques, formule de calcul exacte du montant en espèces à verser en caisse, 6 onglets chronologiques de vérification, impression thermique du ticket de bilan Bluetooth et export PDF de décharge).
  9. *Sécurité, conformité et profil Chef de recouvrement* : code de consentement journalier matinal (`DailyConsentGuard`), identification du terminal (Device ID), renouvellement de mot de passe obligatoire, mises à jour applicatives et délimitation claire avec l'espace dédié du Chef de recouvrement (`docs/recovery-manager/mobile.md`).
- **Générateur & RAG Index** : Compilation de l'ensemble des guides complets imprimables (`user-guide/docs/print_versions/guide_complet_commercial.md`, etc.) et régénération complète de l'index RAG IA à 74 fragments (`backend/src/main/resources/ai/user-guide-index.json`).
...
- Publication APK MinIO : `curl` forcé en HTTPS (`--proto '=https' --tlsv1.2`) pour le Quality Gate SonarCloud (C Security Rating).
- Publication APK MinIO : buckets Contabo alignés (`elykia-mobile-releases[-test]`, idem customer-space) ; `mc mb` ne fait plus échouer le job si le bucket est déjà provisionné.
...
- Fiche client : photo de profil affichée avec la même logique que la liste (fallback thumb/url/chemin local, `getPhotoUrl`, initiales si absent).
