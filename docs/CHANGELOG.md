...
- **User Guide (Application Mobile - Clients)** : Documentation exhaustive du module Clients (« Mes clientes ») sur mobile dans `user-guide/docs/commercial/mobile_app.md` (menu contextuel d'en-tête permettant de basculer instantanément vers la liste de tournée « Clients à Recouvrer », filtres rapides chips Tous/Crédit en cours/Nouveau/Par Quartier, lecture des badges de cartes, formulaire de création avec photo profil, capture photo de pièce d'identité, âge minimum 18 ans, capture GPS native et garant, fiche détaillée à 3 onglets avec appel direct en 1 clic, reliquat disponible, consultation plein écran de la pièce, carte interactive Leaflet, et timeline des transactions).
