...
- **User Guide (Frontend Web - Tontines Web)** : Refonte complète et enrichissement de la documentation du module Tontine dans `user-guide/docs/commercial/tontine.md` :
  ...
  7. *Journal des collectes (`/tontine/collectes`)* : Suivi exhaustif des encaissements (`ROLE_KPI_FINANCIER_TONTINE_COLLECTE`) par période et commercial.
  8. *Archives & Réinitialisation de campagne (`/tontine/reset-collectes`)* : Arborescence hiérarchique Année $\rightarrow$ Run $\rightarrow$ Fichiers PDF par commercial et quartier (`ROLE_CONSULT_TONTINE_COLLECTION_RESET`), action d'archivage simple, et action de réinitialisation annuelle de fin de campagne remettant à zéro les contributions de la session tout en préservant le fichier des membres.
...
- **Générateur & RAG Index** : Compilation de l'ensemble des guides imprimables (`guide_complet_manager.md`, `guide_complet_commercial.md`, `guide_complet_storekeeper.md`, `guide_complet_recovery_manager.md`) et mise à jour de l'index RAG IA (101 fragments) dans `backend/src/main/resources/ai/user-guide-index.json`.
