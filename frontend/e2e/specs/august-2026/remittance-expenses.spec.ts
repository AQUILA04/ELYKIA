...
test.describe('Remise dépenses net @p0 @web @august-2026 @regression', () => {
  test('W-P0-09/10 onglet remise : net = versé − dépenses, freeze si RECEIVED', async ({ page }) => {
    ...
    const summary = await api.getRemittanceSummary(year, month, startDate, endDate);
    expect(
      (summary.candidateExpenses ?? []).some(
        (expense) => (expense.expenseTypeName ?? '').toLowerCase() === 'approvisionnement',
      ),
      'API summary ne doit pas renvoyer Approvisionnement',
    ).toBe(false);
    ...
    if (expenseCount > 0) {
      const typeLabels = await expenses.locator('.expense-type').allTextContents();
      expect(
        typeLabels.some((label) => label.trim().toLowerCase() === 'approvisionnement'),
        'les dépenses Approvisionnement ne doivent pas apparaître',
      ).toBe(false);
    }
