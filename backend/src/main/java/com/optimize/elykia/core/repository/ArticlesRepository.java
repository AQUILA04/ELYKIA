...
public interface ArticlesRepository extends GenericRepository<Articles, Long> {
    ...
    Page<Articles> findByStockQuantityLessThanEqualAndStockQuantityGreaterThan(Integer stockQuantity, Integer zeroValue,
            Pageable pageable);
    ...
    default Page<Articles> elasticsearchEnabled(String keyword, Pageable pageable) {
        return findAll(getElasticsearchEnabledCriteria(keyword), pageable);
    }
    ...
    default Specification<Articles> getElasticsearchCriteria(String keyword) {
        ...
        // final String searchKeyword = StringUtils.isAlpha(keyword) ?
        // String.format("%%%s%%", keyword.toLowerCase()) : keyword;
    ...
    default Specification<Articles> getElasticsearchEnabledCriteria(String keyword) {
        final String searchKeyword = String.format("%%%s%%", keyword.toLowerCase());
        return (root, query, cb) -> {
            jakarta.persistence.criteria.Predicate p = cb.or(
                    cb.like(cb.lower(root.get("marque")), searchKeyword),
            ...
            return cb.and(p, cb.equal(root.get("state"), State.ENABLED));
        };
