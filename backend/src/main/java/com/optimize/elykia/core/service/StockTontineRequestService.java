...
public class StockTontineRequestService extends GenericService<StockTontineRequest, Long> {
    ...
    public StockTontineRequest deliver(Long id) {
        ...
        if (!insufficientStockItems.isEmpty()) {
            throw new CustomValidationException(
                    "Stock magasin insuffisant pour les articles suivants : " + String.join("| ", insufficientStockItems));
        }
    ...
    public Page<StockTontineRequest> getAll(String collector, Pageable pageable) {
        ...
        if (user.is(UserProfilConstant.PROMOTER)) {
            return ((StockTontineRequestRepository) getRepository()).findByCollectorOrderByIdDesc(user.getUsername(), pageable);
        }

        // Si c'est un magasinier, il voit les demandes Validées ou Livrées (pour préparer la livraison)
        ...
        // Dans l'existant: findByStatusInOrderByIdDesc(List.of(StockRequestStatus.VALIDATED, StockRequestStatus.DELIVERED)
        ...
        // L'existant dit: List.of(StockRequestStatus.CREATED, StockRequestStatus.DELIVERED)
        // C'est un peu étrange qu'il ne voit pas VALIDATED, mais je respecte l'existant.
        // Correction: Si je suis gestionnaire, je dois voir CREATED pour pouvoir valider.
        return ((StockTontineRequestRepository) getRepository()).findByStatusInOrderByIdDesc(
                List.of(StockRequestStatus.CREATED, StockRequestStatus.VALIDATED, StockRequestStatus.DELIVERED), pageable);
    }
}
